...
//! proxies commands from peer to `Server`.
use std::{io, net};
use std::str::FromStr;
...
use chat::server::{self, Server};
use chat::codec::{ChatRequest, ChatResponse, ChatCodec};
...
/// `Session` actor is responsible for tcp peer communications.
pub struct Session {
    /// unique session id
    id: usize,
    /// this is address of chat server
    addr: Addr<Syn, Server>,
...
impl StreamHandler<ChatRequest, io::Error> for Session {
...
impl Session {
    ...
    pub fn new(addr: Addr<Syn,Server>,
               framed: actix::io::FramedWrite<WriteHalf<TcpStream>, ChatCodec>) -> Session {
        Session {id: 0, addr: addr, hb: Instant::now(),
...
pub struct TcpServer {
    chat: Addr<Syn, Server>,
}

impl TcpServer {
    pub fn new(s: &str, chat: Addr<Syn, Server>) {
